...
------------------------------
히로, 어바웃 등등 시맨틱 태그를 사용하는 이유는? 내가 어떻게 설계했는지

다크 모드 설정이 로컬 스토리지에 저장되어서 새로고침해도 유지되는 원리

폼 유효성 검사

DOM 이랑 await 필터, 

플렉스 박스와 그리드 차이, 어느 상황에서 각각을 사용해야 하는지
...
이미지에 의미 있는 alt 속성 존재?

폼 요소에 <라벨>이 올바르게 연결(for-id매칭)

css에서 루트로 색상, 폰트, 간격을 정의하는 이유
외부 스타일시트를 사용하는 이유

다크모드용 css 변수를 별도로 정의했는지, 

네비게이션은 플렉스박스 썼고 프로젝트스 카드는 그리드 사용 이유는?
